# WorkflowEngine

Визуальный редактор игровой логики + автономный плеер. Соединяйте блоки вместо написания кода, расширяйте с помощью Lua-скриптов.

## Что это

Инструмент для построения игровой логики путём перетаскивания узлов и соединения портов, а также автономный плеер для распространения игр.

Основная идея: разбить игровую логику на блоки (узлы), каждый блок — это Lua-скрипт, блоки передают данные через соединения. Используйте для:

- Пошаговых боевых систем
- Idle/кликер игр
- Расчёта навыков/баффов
- Конечных автоматов
- Любой логики, представимой как поток данных

## Скриншот

<img width="1403" height="863" alt="image" src="https://github.com/user-attachments/assets/7201603f-72a7-4035-b66b-c1bc7106df32" />

https://github.com/user-attachments/assets/08793b5b-d584-44a1-b641-9e8912ce3061

## Скачать

Загрузите из [Releases](https://github.com/LegnaOS/workflow-game/releases):

| Файл | Описание |
|------|----------|
| `workflow_engine-*-macos-arm64.tar.gz` | macOS Apple Silicon |
| `workflow_engine-*-macos-x64.tar.gz` | macOS Intel |
| `workflow_engine-*-windows-x64.zip` | Windows 64-bit |

Каждый архив содержит:
- `workflow_engine` - IDE редактор
- `workflow_player` - Автономный плеер
- `scripts/` - Предустановленные скрипты
- `workflows/` - Примеры рабочих процессов

## Быстрый старт

### Использование IDE

1. Скачайте и распакуйте архив для вашей платформы
2. Запустите `workflow_engine`
3. Двойной клик по блоку для добавления на холст
4. Перетаскивайте порты для создания соединений
5. Редактируйте свойства в правой панели
6. `Ctrl+S` — сохранить, `Ctrl+O` — открыть

### Публикация игры

1. Создайте workflow в IDE
2. Нажмите «📦 Публикация» в панели инструментов
3. Введите название игры и выберите папку
4. Автоматически создаётся:
   - Папка `ИмяИгры_publish/`
   - Плеер `workflow_player`
   - Зашифрованный пакет `ИмяИгры.lpack`

### Запуск игры

1. Поместите `workflow_player` и `.lpack` в одну папку
2. Дважды кликните на `workflow_player`
3. При наличии нескольких игр появится меню выбора

## Форматы файлов

| Расширение | Описание | Применение |
|------------|----------|------------|
| `.L` | Открытый JSON | Разработка |
| `.LZ` | AES-шифрование | Защита исходников |
| `.lpack` | Зашифрованный пакет | Автономное распространение |

## Создание блоков

Блоки — это Lua-скрипты. Поместите в папку `scripts/`, автозагрузка с горячей перезагрузкой.

```lua
return {
    meta = {
        id = "my.double",
        name = "Удвоение",
        category = "Математика",
        color = "#FF5722"
    },
    inputs = {
        { id = "value", name = "Вход", type = "number", default = 0 }
    },
    outputs = {
        { id = "result", name = "Результат", type = "number", default = 0 }
    },
    execute = function(self, inputs)
        return { result = inputs.value * 2 }
    end
}
```

Подробнее: [docs/BLOCK_DEVELOPMENT_RU.md](docs/BLOCK_DEVELOPMENT_RU.md)

## Встроенные скрипты

```
scripts/
├── lite/          # Lite RPG Idle-игра
│   ├── hero       # Герой
│   ├── boss       # Босс
│   ├── weapon     # Оружие
│   ├── armor      # Броня
│   ├── skill      # Навык
│   └── gem_*      # Камни (атака/крит/уклонение)
├── game/          # Игровое ядро
│   ├── character  # Персонаж
│   ├── monster    # Монстр
│   ├── attack     # Расчёт атаки
│   └── ...
├── logic/         # Логика
│   ├── branch     # Условное ветвление
│   ├── compare    # Сравнение
│   └── selector   # Селектор
├── math/          # Математика
│   ├── add        # Сложение
│   ├── multiply   # Умножение
│   └── calc       # Выражение
├── input/         # Интерактивный ввод
│   ├── text_input # Текстовое поле
│   ├── password   # Поле пароля
│   └── button     # Кнопка
└── util/          # Утилиты
    ├── splitter   # Разделитель
    ├── merger     # Объединитель
    └── switch     # Переключатель
```

## Сборка

Требуется Rust 1.70+

```bash
# Разработка
cargo run

# Сборка IDE и плеера
cargo build --release

# Результат
target/release/workflow_engine  # IDE
target/release/workflow_player  # Плеер
```

## Структура проекта

```
src/
├── main.rs           # Точка входа IDE
├── player.rs         # Точка входа плеера
├── app.rs            # Основная логика
├── script/           # Lua-движок
│   ├── parser.rs     # Парсинг скриптов
│   ├── registry.rs   # Реестр блоков
│   └── loader.rs     # Обработка кодировки
├── workflow/         # Ядро workflow
│   ├── graph.rs      # Структура графа
│   ├── block.rs      # Определение блока
│   ├── connection.rs # Соединения
│   ├── package.rs    # Формат игрового пакета
│   └── storage.rs    # Хранилище
└── ui/               # Компоненты UI
    ├── canvas.rs     # Холст
    ├── block_widget.rs
    └── connection_widget.rs
```

## Технологии

- **Rust** — основной язык
- **egui/eframe** — immediate mode GUI
- **mlua** — Lua 5.4
- **aes/cbc** — AES-128-CBC шифрование
- **serde** — сериализация

## Лицензия

MIT

